# Geometria Superior - Primer Parcial 30/04/2026
...
>>	3. **Diferenciabilidad** Sea $q\in C$ arbitrario. Como $\pi:\mathbb R^2\to C$ es sobreyectiva, existe $(x_0,y_0)\in\mathbb R^2$ tal que $$\pi(x_0,y_0)=q.$$
>>	4. Tomamos un abierto $$U=(x_0-\varepsilon,x_0+\varepsilon)\times(y_0-\varepsilon,y_0+\varepsilon)$$con $0<\varepsilon<1$.  
>>	5. Por lo visto en el práctico, los pares $$\left(\pi(U),(\pi|_U)^{-1}\right)$$dan una estructura diferenciable en el cociente $C=\mathbb R^2/\sim$.
>>	6. En particular, $$\varphi=(\pi|_U)^{-1}:\pi(U)\to U\subset\mathbb R^2$$es una carta suave de $C$ alrededor de $q$.  
>>	7. Para ver que $f$ es diferenciable en $q$, calculamos su representación local en esta carta: $$f\circ\varphi^{-1}:U\subseteq \mathbb{R}^{2}\to\mathbb R.$$
>>	8. Como $\varphi^{-1}=\pi|_U$, se tiene $$f\circ\varphi^{-1}=f\circ\pi|_U.$$
>>	9. Luego, para todo $(x,y)\in U$, $$f\circ\varphi^{-1}(x,y)=f(\pi(x,y))=e^y\sin(5\pi x).$$
>>	10. Pero la función $$h:\mathbb R^2\to\mathbb R,\qquad h(x,y)=e^y\sin(5\pi x)$$es suave. Por lo tanto, su restricción $$h|_U=f\circ\varphi^{-1}$$también es suave.  
>>	11. Como $q\in C$ era arbitrario, $f$ es diferenciable en todo punto. Entonces $$f:C\to\mathbb R$$es diferenciable.
>>- (b)

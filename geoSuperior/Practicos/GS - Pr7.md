...
>[!Exercise]
>Let $M$ be an oriented manifold and let $(U,\varphi)$ be a chart of $M$ with $U$ connected. Then $(U,\varphi)$ is either positively oriented everywhere on $U$ or negatively oriented everywhere on $U$.
>
>>[!Proof]-
>>21. Let $\mathcal A$ be an oriented atlas determining the orientation of $M$. For each $p\in U$, choose a chart $(V,\psi)\in\mathcal A$ with $p\in V$.
>>22. Define $A\subseteq U$ by saying that $p\in A$ if the basis induced by $(U,\varphi)$ at $p$ has the same orientation as the basis induced by $(V,\psi)$ at $p$. Equivalently, if $\varphi=(x^1,\ldots,x^n)$ and $\psi=(y^1,\ldots,y^n)$, then $p\in A$ when the change-of-basis determinant between $\left(\frac{\partial}{\partial x^1}\big|_p,\ldots,\frac{\partial}{\partial x^n}\big|_p\right)$ and $\left(\frac{\partial}{\partial y^1}\big|_p,\ldots,\frac{\partial}{\partial y^n}\big|_p\right)$ is positive.
>>23. This definition does not depend on the chosen oriented chart $(V,\psi)$. Indeed, if $(W,\theta)\in\mathcal A$ is another oriented chart with $p\in W$, then the change-of-basis determinant from the $\psi$-basis to the $\theta$-basis is positive because both charts belong to the oriented atlas. Hence the sign of the comparison between the $\varphi$-basis and the oriented basis is unchanged.
>>24. Define $B\subseteq U$ analogously by saying that $p\in B$ if the basis induced by $(U,\varphi)$ at $p$ has the opposite orientation from the given orientation of $M$ at $p$. Then $A\cap B=\varnothing$.
>>25. Also $U=A\cup B$. Indeed, for every $p\in U$, the transition determinant comparing the $\varphi$-basis with an oriented basis is nonzero, so it is either positive or negative.
>>26. We prove that $A$ is open. Let $p\in A$ and choose $(V,\psi)\in\mathcal A$ with $p\in V$. On $U\cap V$, the determinant function comparing the $\varphi$-basis with the $\psi$-basis is smooth and nonzero. Since it is positive at $p$, by continuity there exists an open neighborhood $N\subseteq U\cap V$ of $p$ such that the determinant remains positive on $N$. Therefore $N\subseteq A$, so $A$ is open in $U$.
>>27. The same argument proves that $B$ is open in $U$: if $p\in B$, the corresponding determinant is negative at $p$, so by continuity it remains negative on some neighborhood of $p$ inside $U$.
>>28. Thus $U=A\cup B$, where $A$ and $B$ are disjoint open subsets of $U$. Since $U$ is connected, one of them must be empty.
>>29. Therefore either $A=U$ or $B=U$. Hence $(U,\varphi)$ is either positively oriented everywhere on $U$ or negatively oriented everywhere on $U$.
...
>>[!Proof]-
>>- (a)
>>1. Assume, by contradiction, that $M$ is orientable. Let $\mathcal A$ be an oriented atlas defining the orientation of $M$.
>>2. Let $(U,\varphi)$ and $(V,\psi)$ be the two coordinate charts, and for every $r\in U\cap V$ denote their coordinate bases by $\mathcal B_\varphi(r)$ and $\mathcal B_\psi(r)$.
>>3. By hypothesis, after naming the two connected components of $U\cap V$ as $C_1$ and $C_2$, we have $\det C_{\mathcal B_\varphi(r)\mathcal B_\psi(r)}>0$ for every $r\in C_1$, and $\det C_{\mathcal B_\varphi(r)\mathcal B_\psi(r)}<0$ for every $r\in C_2$.
>>4. Since $U$ is connected, [[GS - Pr7#^44f51c|Ejercicio]] implies that the chart $(U,\varphi)$ is either positive or negative with respect to the orientation induced by $\mathcal A$.
>>5. Therefore, for any oriented chart $(W,\theta)\in\mathcal A$ with $r\in W$, exactly one of the following alternatives holds on all of $U$: either $\det C_{\mathcal B_\varphi(r)\mathcal B_\theta(r)}>0$ for every $r\in U$, or $\det C_{\mathcal B_\varphi(r)\mathcal B_\theta(r)}<0$ for every $r\in U$.
>>6. Similarly, since $V$ is connected, Exercise 4 implies that the chart $(V,\psi)$ is either positive or negative with respect to the orientation induced by $\mathcal A$.
>>7. Therefore, for any oriented chart $(W,\theta)\in\mathcal A$ with $r\in W$, exactly one of the following alternatives holds on all of $V$: either $\det C_{\mathcal B_\psi(r)\mathcal B_\theta(r)}>0$ for every $r\in V$, or $\det C_{\mathcal B_\psi(r)\mathcal B_\theta(r)}<0$ for every $r\in V$.
>>8. Now fix $r\in U\cap V$ and choose an oriented chart $(W,\theta)\in\mathcal A$ with $r\in W$. Then the change of basis from $\mathcal B_\varphi(r)$ to $\mathcal B_\psi(r)$ factors through $\mathcal B_\theta(r)$: $$C_{\mathcal B_\varphi(r)\mathcal B_\psi(r)}=C_{\mathcal B_\theta(r)\mathcal B_\psi(r)}C_{\mathcal B_\varphi(r)\mathcal B_\theta(r)}$$
>>9. Taking determinants, $$\det C_{\mathcal B_\varphi(r)\mathcal B_\psi(r)}=\det C_{\mathcal B_\theta(r)\mathcal B_\psi(r)}\det C_{\mathcal B_\varphi(r)\mathcal B_\theta(r)}$$
>>10. Since $C_{\mathcal B_\theta(r)\mathcal B_\psi(r)}=\left(C_{\mathcal B_\psi(r)\mathcal B_\theta(r)}\right)^{-1}$, we get $$\det C_{\mathcal B_\theta(r)\mathcal B_\psi(r)}=\frac{1}{\det C_{\mathcal B_\psi(r)\mathcal B_\theta(r)}}$$
>>11. Hence $$\det C_{\mathcal B_\varphi(r)\mathcal B_\psi(r)}=\frac{\det C_{\mathcal B_\varphi(r)\mathcal B_\theta(r)}}{\det C_{\mathcal B_\psi(r)\mathcal B_\theta(r)}}$$
>>12. The numerator is either always $>0$ or always $<0$ on $U$, and the denominator is either always $>0$ or always $<0$ on $V$. Therefore the quotient is either always $>0$ or always $<0$ on all of $U\cap V$.
>>13. Thus $\det C_{\mathcal B_\varphi(r)\mathcal B_\psi(r)}$ must have one fixed sign on all of $U\cap V$.
>>14. This contradicts Step 3, because the same determinant is positive on $C_1$ and negative on $C_2$.
>>15. Therefore $M$ is not orientable.
>>- (b)
...
>>29. Finalmente, $$\det[v_1,\ldots,v_n]=\Omega_p(v_1,\ldots,v_n).$$
>>30. Luego $$(f^*\Omega)_p(v_1,\ldots,v_n)=\Omega_p(v_1,\ldots,v_n)$$ para todo $p\in\mathbb R^n$ y todo $v_1,\ldots,v_n\in T_p\mathbb R^n$. Por lo tanto $$f^*\Omega=\Omega.$$
...
>
>>[!Proof]-
>>31. Escribimos $\varphi=(y_1,\ldots,y_n)$. Como $F:M\to N$ es difeomorfismo y $V\subseteq N$ es abierto, $F^{-1}(V)\subseteq M$ es abierto. Además $\varphi\circ F:F^{-1}(V)\to \varphi(V)$ es un homeomorfismo sobre el abierto $\varphi(V)\subseteq \mathbb R^n$, y es suave con inversa suave porque $F$ y $\varphi$ lo son. Por lo tanto $(F^{-1}(V),\varphi\circ F)$ es una carta suave de $M$.
>>32. Para probar que esta carta es positiva, fijamos $p\in F^{-1}(V)$. Denotamos por $z_i=y_i\circ F$ las funciones coordenadas de la carta $\varphi\circ F$. Entonces la base coordenada asociada a esta carta en $p$ es $$B=\left(\frac{\partial}{\partial z_1}\big|_p,\ldots,\frac{\partial}{\partial z_n}\big|_p\right)$$
...
>>34. Como $(V,\varphi)$ es una carta positiva en $N$, la base $\left(\frac{\partial}{\partial y_1}\big|_{F(p)},\ldots,\frac{\partial}{\partial y_n}\big|_{F(p)}\right)$ es positiva en $T_{F(p)}N$. Por el paso anterior, esta base es exactamente $(dF)_p(B)$.
>>35. Por lo tanto $B$ es positiva (de lo contrario $dF_{p}$ mandaria una base negativa a una base positiva, lo que es absurdo por que $F$ preserva orientacion)
>>36. Así, para todo $p\in F^{-1}(V)$, la base coordenada de la carta $(F^{-1}(V),\varphi\circ F)$ es positiva en $T_pM$. Por lo tanto $(F^{-1}(V),\varphi\circ F)$ es un entorno coordenado positivo en $M$.
...
>>14. Entonces por el [[GS - Pr7#^e4aa86|Ejercicio 10]] sabemos que $$(d\pi)_{p}\left( \frac{\partial}{\partial z^{i}}\bigg|_{p}\right)=\frac{\partial}{\partial y^{i}}\bigg|_{p}$$ por ende $[(d\pi)_{p}]_{\widetilde {\mathcal{B}}_{1}}^{\widetilde{\mathcal{B}}_{2}}=Id$
...
>
>>[!Proof]-
>>6. En las dos componentes de $U\cap V$ se cumple $$(s\circ t^{-1})(r)=\begin{cases}r,&r\in(0,\pi),\\r+2\pi,&r\in(-\pi,0).\end{cases}$$ En ambos casos, $\dfrac{d}{dr}(s\circ t^{-1})(r)=1>0$, por lo que las cartas $(U,s)$ y $(V,t)$ pertenecen a la misma orientación. Notando que $\dfrac{d}{dr}(s\circ t^{-1})(r)=1>0$ es el unico coeficiente en la matriz cambio de base entre $\frac{d}{dt}$ y $\frac{d}{ds}$
>>7. Sea $\{\rho_U,\rho_V\}$ una partición de la unidad subordinada al cubrimiento $\{U,V\}$. Entonces $\rho_U+\rho_V=1$ en $S^1$. Por definición de integral sobre una variedad, $$\int_{S^1}\theta=\int_U\rho_U\theta+\int_V\rho_V\theta.$$
>>8. Como $\theta|_U=ds$ y $\theta|_V=dt$, entonces la funcion coeficiente que acompaña ambas formas es constantemente $1$, entonces pasando a coordenadas obtenemos $$\int_{S^1}\theta=\int_0^{2\pi}(\rho_U\circ s^{-1})(u)\,du+\int_{-\pi}^{\pi}(\rho_V\circ t^{-1})(r)\,dr.$$
>>9. Partimos ambas integrales según las dos componentes del solapamiento: $$\begin{aligned}\int_{S^1}\theta&=\int_0^\pi(\rho_U\circ s^{-1})(u)\,du+\int_\pi^{2\pi}(\rho_U\circ s^{-1})(u)\,du\\&\quad+\int_{-\pi}^{0}(\rho_V\circ t^{-1})(r)\,dr+\int_0^\pi(\rho_V\circ t^{-1})(r)\,dr.\end{aligned}$$
...
>>11. En la integral sobre $(-\pi,0)$ hacemos el cambio de variable $u=r+2\pi$, de modo que $r=u-2\pi$ y $dr=du$. Como $r\in(-\pi,0)$ si y solo si $u\in(\pi,2\pi)$, resulta $$\int_{-\pi}^{0}(\rho_V\circ t^{-1})(r)\,dr=\int_\pi^{2\pi}(\rho_V\circ t^{-1})(u-2\pi)\,du.$$
>>12. Para $u\in(\pi,2\pi)$ se cumple $s^{-1}(u)=t^{-1}(u-2\pi)$, pues ambas expresiones representan el mismo punto de la parte inferior de $S^1$. Evaluando $\rho_U+\rho_V=1$ en ese punto común obtenemos $$\rho_U(s^{-1}(u))+\rho_V(t^{-1}(u-2\pi))=1,$$ es decir, $$(\rho_U\circ s^{-1})(u)+(\rho_V\circ t^{-1})(u-2\pi)=1.$$ Por tanto, $$\begin{aligned}\int_\pi^{2\pi}(\rho_U\circ s^{-1})(u)\,du+\int_{-\pi}^{0}(\rho_V\circ t^{-1})(r)\,dr&=\int_\pi^{2\pi}\left[(\rho_U\circ s^{-1})(u)+(\rho_V\circ t^{-1})(u-2\pi)\right]du\\&=\int_\pi^{2\pi}1\,du=\pi.\end{aligned}$$
>>13. Sumando ambas contribuciones obtenemos $$\int_{S^1}\theta=\pi+\pi=2\pi.$$ Por lo tanto, $$\boxed{\int_{S^1}\theta=2\pi}.$$
...
>
>>[!Proof]-
>>14. Como $p_1^*\omega_1\in\Omega^{n_1}(M_1\times M_2)$ y $p_2^*\omega_2\in\Omega^{n_2}(M_1\times M_2)$, se tiene $$\omega=(p_1^*\omega_1)\wedge(p_2^*\omega_2)\in\Omega^{n_1+n_2}(M_1\times M_2).$$
>>15. Fijemos un punto arbitrario $(p,q)\in M_1\times M_2$. Tomemos una carta $(U,x=(x^1,\ldots,x^{n_1}))$ de $M_1$ alrededor de $p$ y una carta $(V,y=(y^1,\ldots,y^{n_2}))$ de $M_2$ alrededor de $q$.
>>16. Como $\omega_1$ y $\omega_2$ son formas de grado máximo, en estas cartas existen funciones suaves $f:U\to\mathbb R$ y $g:V\to\mathbb R$ tales que $$\omega_1=f\,dx^1\wedge\cdots\wedge dx^{n_1},\qquad \omega_2=g\,dy^1\wedge\cdots\wedge dy^{n_2}.$$
>>17. Como $\omega_1$ y $\omega_2$ nunca se anulan, se cumple $$f(p)\neq 0,\qquad g(q)\neq 0.$$
>>18. Usando las propiedades $F^*(h\alpha)=(h\circ F)F^*\alpha$ y $F^*(\alpha\wedge\beta)=F^*\alpha\wedge F^*\beta$, obtenemos $$p_1^*\omega_1=(f\circ p_1)\,p_1^*(dx^1)\wedge\cdots\wedge p_1^*(dx^{n_1})$$ y $$p_2^*\omega_2=(g\circ p_2)\,p_2^*(dy^1)\wedge\cdots\wedge p_2^*(dy^{n_2}).$$
>>19. Por lo tanto, $$\omega=(f\circ p_1)(g\circ p_2)\,p_1^*(dx^1)\wedge\cdots\wedge p_1^*(dx^{n_1})\wedge p_2^*(dy^1)\wedge\cdots\wedge p_2^*(dy^{n_2}).$$
>>20. La carta producto $(U\times V,x\times y)$ determina en $T_{(p,q)}(M_1\times M_2)$ la base $$\left(\left(\frac{\partial}{\partial x^1}\Big|_p,0\right),\ldots,\left(\frac{\partial}{\partial x^{n_1}}\Big|_p,0\right),\left(0,\frac{\partial}{\partial y^1}\Big|_q\right),\ldots,\left(0,\frac{\partial}{\partial y^{n_2}}\Big|_q\right)\right).$$
>>21. Ya sabemos que las diferenciales de las proyecciones satisfacen $$(dp_1)_{(p,q)}\left(\frac{\partial}{\partial x^j}\Big|_p,0\right)=\frac{\partial}{\partial x^j}\Big|_p,\qquad (dp_1)_{(p,q)}\left(0,\frac{\partial}{\partial y^j}\Big|_q\right)=0,$$ y análogamente $$(dp_2)_{(p,q)}\left(\frac{\partial}{\partial x^j}\Big|_p,0\right)=0,\qquad (dp_2)_{(p,q)}\left(0,\frac{\partial}{\partial y^j}\Big|_q\right)=\frac{\partial}{\partial y^j}\Big|_q.$$
>>22. Por definición del pullback de una $1$-forma, para todo $Z\in T_{(p,q)}(M_1\times M_2)$ se tiene $$(p_1^*dx^i)_{(p,q)}(Z)=(dx^i)_p\bigl((dp_1)_{(p,q)}Z\bigr).$$
>>23. En consecuencia, $$(p_1^*dx^i)_{(p,q)}\left(\frac{\partial}{\partial x^j}\Big|_p,0\right)=(dx^i)_p\left(\frac{\partial}{\partial x^j}\Big|_p\right)=\delta^i_j,$$ mientras que $$(p_1^*dx^i)_{(p,q)}\left(0,\frac{\partial}{\partial y^j}\Big|_q\right)=(dx^i)_p(0)=0.$$
>>24. De la misma manera, $$(p_2^*dy^i)_{(p,q)}\left(\frac{\partial}{\partial x^j}\Big|_p,0\right)=0,$$ y $$(p_2^*dy^i)_{(p,q)}\left(0,\frac{\partial}{\partial y^j}\Big|_q\right)=(dy^i)_q\left(\frac{\partial}{\partial y^j}\Big|_q\right)=\delta^i_j.$$
>>25. Usamos ahora la propiedad de menores: si $\alpha_1,\ldots,\alpha_k$ son $1$-formas, entonces $$(\alpha_1\wedge\cdots\wedge\alpha_k)(v_1,\ldots,v_k)=\det\bigl(\alpha_i(v_j)\bigr).$$
>>26. Evaluando $\omega_{(p,q)}$ sobre la base producto del paso 7 obtenemos $$\begin{aligned}&\omega_{(p,q)}\left(\left(\frac{\partial}{\partial x^1}\Big|_p,0\right),\ldots,\left(\frac{\partial}{\partial x^{n_1}}\Big|_p,0\right),\left(0,\frac{\partial}{\partial y^1}\Big|_q\right),\ldots,\left(0,\frac{\partial}{\partial y^{n_2}}\Big|_q\right)\right)\\&=(f\circ p_1)(p,q)(g\circ p_2)(p,q)\det\begin{pmatrix}I_{n_1}&0\\0&I_{n_2}\end{pmatrix}\\&=f(p)g(q).\end{aligned}$$
>>27. Como $f(p)\neq 0$ y $g(q)\neq 0$, resulta $$\omega_{(p,q)}\neq 0.$$
>>28. Puesto que $(p,q)$ era arbitrario, $\omega$ nunca se anula en $M_1\times M_2$.
>>29. Por lo tanto, $\omega$ es una forma de orientación sobre $M_1\times M_2$, y en consecuencia $M_1\times M_2$ es orientable.

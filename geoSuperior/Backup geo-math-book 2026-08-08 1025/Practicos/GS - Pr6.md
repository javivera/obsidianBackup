...
>>	8. Como $F(x,y,z)=x^2yz$ tiene $dF=(2xyz)dx+(x^2z)dy+(x^2y)dz$, y en el octante positivo $dF\neq 0$, cada $$S_k=\{(x,y,z)\in\mathbb R^3_{>0}:x^2yz=k\}$$ con $k>0$ es una superficie regular. (Por teorema de rango constante, para cualquier $p\in S_{k}=F^{-1}(k)$ tenemos que $dF_{p}$ es sobreyectiva, por ser distinta de nula y codominio de dimension 1) 
>>	9. Además, $$dF_{(x,y,z)}X_{(x,y,z)}=X_{(x,y,z)}(F)=x(2xyz)-2y(x^2z)=0$$ y $$dF_{(x,y,z)}Y_{(x,y,z)}=Y_{(x,y,z)}(F)=xy(x^2z)-xz(x^2y)=0$$
>>	10. Luego $X_q,Y_q\in \ker(dF_{q})=d\iota_{q}(T_qS_k)\simeq T_{q}S_{k}$ para todo $q\in S_k$. Osea $$\operatorname{span} \{ X_{q},Y_{q} \}\subseteq d\iota_{q}(T_{q}S_{k})$$ 
>>	11. Como $\mathcal D_q=\operatorname{span}\{ X_q,Y_q \}$ es de dimensión $2$ y $d\iota_{q}(T_{q}S_{k})$ también es de dimensión $2$ (por que $T_qS_k$ lo és) y usando la inclusión del paso 10. se concluye $$\mathcal D_q=T_qS_k$$
>>	12. Por lo tanto, las superficies integrales son $$S_k=\{(x,y,z)\in\mathbb R^3_{>0}:x^2yz=k\}$$, con $k>0$.

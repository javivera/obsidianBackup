...
>>8. Por lo tanto, para todo $t\in I$, $$\sigma'(t)=0=X_{\sigma(t)}$$luego $\sigma$ es curva integral de $X$.
>>9. Además, $\sigma(t_0)=p=\gamma(t_0)$.
>>10. Luego por [[Glosario#^c00595z]], como $\gamma$ y $\sigma$ son curvas integrales de $X$ y coinciden en el instante $t_0$, se sigue que $\gamma(t)=\sigma(t)$ para todo $t\in I$.
>>11. Como $\sigma(t)=p$ para todo $t\in I$, concluimos que $\gamma(t)=p$ para todo $t\in I$.
>>12. Por lo tanto,
...
>>
>>2. Supongamos, por contradicción, que $J$ tiene extremo superior finito $b<\infty$. Tomemos $t_0\in J$. Como $M$ es compacta, $$\gamma([t_0,b))\subseteq M$$osea $\gamma([t_{0},b))$ está contenida en un compacto. 
>>3. Por el [[GS - Teo17#^f386fd|Lema de escape]], una curva integral maximal cuyo extremo superior es finito no puede permanecer contenida en un compacto. Contradicción.
>>
>>4. Análogamente, $J$ no puede tener extremo inferior finito. 
>>5. Por tanto, $J=\mathbb R$ osea
>>$$X \text{ es completo.}$$
...
>>[!Proof]-
>>- **(a)**
>>	1. Primero notamos que como $\phi$ es inmersion tiene rango constante por que $$d\phi:T_{p}M\rightarrow T_{\phi(p)}N$$ es inyectiva para todo $p\in M$ entonces su $\ker$ tiene dimension nula por ende  $$\dim Im(d\phi_{p})=\dim T_{p}M=m$$ para todo $p\in M$
...
>>	3. Luego si evaluamos en $\varphi(p)=(u_{1}(p),\ldots,u_{m}(p))$ tenemos $$\begin{align*}\bigl(v_1(\phi(p)),\ldots,v_n(\phi(p))\bigr)&=\psi\circ\phi(p)\\&=\psi\circ\phi\circ\varphi^{-1}\bigl(\varphi(p)\bigr)\\&=\bigl(u_1(p),\ldots,u_m(p),0,\ldots,0\bigr)\end{align*}$$
...
>>	6. Mostrando finalmente que $$d\phi_{p}\left(\frac{\partial}{\partial u_{i}}\bigg|_{p}\right)=\frac{\partial}{\partial v_{i}}\bigg|_{\phi(p)}$$ y ademas que $\operatorname{Im}(d\phi_{p})=\operatorname{span}\left\{  \frac{d}{dv_{1}}\big|_{\phi(p)}\ldots \frac{d}{dv_{m}}\big|_{\phi(p)}  \right\}$ 
...
>>	8. Por otro lado pedimos que se cumpla $$(d\phi)_{p}(X_{p})=Y_{\phi(p)}=\sum^{n}_{j=1}c_{j}(\phi(p))\frac{\partial}{\partial v_{j}}\bigg|_{\phi(p)}=\sum^{m}_{j=1}c_{j}(\phi(p))\frac{\partial}{\partial v_{j}}\bigg|_{\phi(p)}$$ el ultimo igual es por que $$Y_{\phi(p)}\in \operatorname{Im}(d\phi_{p})=\operatorname{span}\left\{  \frac{d}{dv_{1}}\bigg|_{\phi(p)}\ldots \frac{d}{dv_{m}}\bigg|_{\phi(p)}  \right\}$$ 
>>	9. Y eso se cumple si $b_{j}(p)=c_{j}(\phi(p))$. Como $Y$ es suave $c_{j}$ es suave y $\phi$ es suave por definicion luego $b_{j}$ son suaves. Mostrando que $X|_{U}$ es suave.
...
>>	4. Entonces por (a) existe un campo $X$ que esta dado por $$(d\iota)_{p}(X_{p})=Y_{\iota (p)}$$ 
...
>>2. Por [[GS - Teo18#^d0b758|Teorema de extension local de campos]], para cada $p\in S$ existen un abierto $U_p\subseteq S$ con $p\in U_p$, un abierto $V_p\subseteq M$ con $p\in V_p$ y un campo suave $Y^{(p)}\in\mathfrak X(V_p)$ tales que $$Y^{(p)}_q=(di)_qX_q,\qquad q\in U_p.$$
...
>>4. El conjunto $\{V_p\}_{p\in S}$ es un cubrimiento abierto de $S$. Tomamos una partición de la unidad suave $\{\rho_\alpha\}_{\alpha\in A}$ subordinada a este cubrimiento. Para cada $\alpha\in A$, existe $p_\alpha\in S$ tal que $$\operatorname{supp}(\rho_\alpha)\subseteq V_{p_\alpha}.$$
>>5. Nos gustaria sobre $W=\bigcup_{p\in S}V_{p}$ definir el campo $$Y:=\sum_{\alpha\in A}\rho_\alpha Y^{(p_\alpha)}.$$esta suma está bien definida y define un campo suave porque la familia $\{\rho_\alpha\}$ es localmente finita. En efecto, para cada $q\in W$ existe un entorno $O_q$ en el cual solo una cantidad finita de funciones $\rho_\alpha$ es no nula. Por lo tanto, sobre $O_q$, el campo $Y$ es una suma finita de campos suaves.
>>6. El problema que tenemos es que podria suceder que $Y_{p}\neq X_{p}$ para algun $q\in S$. Por ejemplo si $q\in S$ y $q\in V_{p_{1}}$ y $q\in U_{p_{1}}$ pero $q\in V_{p_{2}}$ pero $q\not\in U_{p_{2}}$ entonces podriamos afirmar $Y_{q} ^{(p_{1})}=X_{q}$ pero no necesariamente $Y_{q}^{(p_{2})}=X_{q}$ entonces en esa parte la suma (solo a modo de ejemplo supongamos que son $q$ esta solamente en esos dos conjuntos)  $$Y_{q}=\rho_{1}(q)Y_{q}^{(p_{1})}+\rho_{2}(q)Y_{q}^{(p_{2})}=\frac{1}{2}X_{q}+\frac{1}{2} K$$ que podria ser distinta que $X_{q}$ 
>>7. Para solucionar esto podemos achicar cada $V_p$ (conservando la notacion por comodidad) , si es necesario, de modo que $$V_p\cap S\subseteq U_p.$$ osea que $V_{p}$ extienda a $U_{p}$ pero por fuera de $S$, dentro de $S$ coincide con $U_{p}$
>>8. En efecto esto se puede hacer porque como $U_p$ es abierto en $S$ y $S=i(S)$ tiene la topologia heredada de $M$, existe un abierto $O_p\subseteq M$ tal que $U_p=O_p\cap S$, y podemos reemplazar $V_p$ por $V_p\cap O_p$. Con lo cual $$V_{p}\cap O_{p}\cap S=V_{p}\cap U_{p}\subseteq U_{p}$$ como queriamos
>>9. Ahora si, sea $$W:=\bigcup_{p\in S}V_p.$$ Entonces $W$ es un abierto de $M$ que contiene a $S$, es decir, una vecindad abierta de $S$.
>>10. Veamos que $Y$ restringido a $S$ coincide con $X$. Sea $q\in S$. Si $\rho_\alpha(q)\neq 0$, entonces $$q\in\operatorname{supp}(\rho_\alpha)\subseteq V_{p_\alpha}.$$ Como además $q\in S$, resulta $$q\in V_{p_\alpha}\cap S\subseteq U_{p_\alpha}.$$ Por lo tanto, $$Y^{(p_\alpha)}_q=X_q.$$
>>11. En consecuencia, usando que $\{\rho_\alpha\}$ es una partición de la unidad, $$Y_q=\sum_{\alpha\in A}\rho_\alpha(q)Y^{(p_\alpha)}_q=\sum_{\alpha\in A}\rho_\alpha(q)X_q=\left(\sum_{\alpha\in A}\rho_\alpha(q)\right)X_q=X_q.$$
>>12. Así, $Y\in\mathfrak X(W)$ es un campo suave definido sobre una vecindad $W$ de $S$ y satisface $$Y|_S=X.$$

...
>[!Proposition] Proy sobre un convexo
...
>>[!Proof]-
>>- Existencia
>>	1. $\gamma = \inf\{ \lVert p-c \rVert:c\in A \}$ usar def de infimo $\gamma^{2}\leq\lVert p-q_{n} \rVert^{2} \leq \gamma^{2} + \frac{1}{n}$
>>	2. Veamos $q_{n}$ es cauchy    $$\begin{align}\lVert 2p-(q_{n}-q_{m})\rVert^2+\lVert q_{n}-q_{m} \rVert &  = \lVert (p-q_{n}) + (p-q_{m})\rVert^2 + \lVert (p-q_{n})-(p-q_{m}) \rVert^2
>> 	  \\& = 2\lVert p-q_{n} \rVert^2+2\lVert p-q_{m} \rVert^2 \quad\text{(Regla Paralelogramo) }\\& \leq 4\gamma^2+2\left( \frac{1}{n}+\frac{1}{m} \right)\end{align}$$
>>	3. Como $A$ conexo $\frac{1}{2}(q_{n}+q_{m})\in A$ entonces  $$4\gamma^2 \leq 4\left\lVert  p-\frac{1}{2}(q_{n}+q_{m})  \right\rVert^2 = \lVert 2p-(q_{n}+q_{m}) \rVert^2$$ (Por 1.)
>>	4. $\lVert q_{n}-q_{m} \rVert^{2} < 2\left( \frac{1}{n} +\frac{1}{m}\right)$. Mostrando que es de Cauchy
>>	5. Converge por que $H$ Hilbert. Ademas $A$ cerrado entonces $\lim\limits_{ n \to \infty }q_{n}=q\in A$
>>	6. $\gamma^2 \leq \lVert p-q \rVert^2=\lim_{ n \to \infty }\lVert p-q_{n} \rVert^2\leq\lim_{ n \to \infty }\gamma^2 + \frac{1}{n}=\gamma^2$ (Norma es continua entonces limite entra)
>>
>>- Unicidad
>>	1. Supongo $w\in A$ tal que $\lVert p-w \rVert=\gamma$
>>	2. Entonces $\frac{1}{2}(q+w)\in A$ digamos $$\left\lVert  p- \frac{1}{2}(q+w)  \right\rVert^{2}\geq\gamma^{2}$$
>>	3. Luego $$\begin{align}\lVert 2p-(q+w) \rVert^{2}+\lVert q-w \rVert^{2}  & =\lVert (p-w) + (p-q) \rVert^2 + \lVert (p-w)-(p-q) \rVert^2 \\&=2\lVert p-w \rVert^{2}+2\lVert p-q \rVert^{2}\quad (\text{Regla Paralelogramo})\\&=4\gamma^2\end{align}$$
>>	4. Finalmente $$\lVert q-w \rVert^2=4\gamma^2-4\left\lVert  p- \frac{1}{2}(q+w)  \right\rVert^{2}\leq 0$$

^de887b

>[!Remark]
>Sea $p \in \mathcal{H}$ y $q \in A$ que satisface [[#^de887b]] convexo y $\mathbb{F}=\mathbb{R}$ entonces $$\|p-q\| = \min_{a \in A} \|p-a\| \iff ( p-q, a-q ) \le 0\quad\forall a\in A$$
>>[!Proof]-
>>- (Ida) 
...
>>	$$\|(p-q) - t(a-q)\|^2 = \|p-q\|^2 - 2t( p-q, a-q ) + t^2\|a-q\|^2$$
>>	4. Por lo tanto $$0 \le t^{2}\|a-q\|^2 - 2( p-q, a-q )$$
>>	5. Usando $t\rightarrow 0$ concluimos $0 \geq ( p-q, a-q )$
>>
>>- (Vuelta) 
>>	1. Devuelta usando $\mathbb{F}=\mathbb{R}$ $$\begin{align}\|p-a\|^2 & = \|(p-q)-(a-q)\|^2\\&= \|p-q\|^2 + \|a-q\|^2 - 2( p-q, a-q )\\&\ge \|p-q\|^2 \quad \forall a \in A\end{align}$$
...
>Sea $Y$ subespacio cerrado en $\mathcal{H}$ Hilbert entonces $\forall x \in \mathcal{H}$ existe único $y\in Y,z\in Y^{\perp}$ tales que $x=y+z$. Ademas $\lVert x \rVert^2=\lVert y \rVert^2+\lVert z \rVert^2$ (Generalizacion de Pitagoras)
>>[!Proof]-
>>- Existencia
>>	1. $x \in \mathcal{H}$ e $Y\neq \emptyset$ convexo ([[Teórico 3#^688643]])  y cerrado entonces por [[Teórico 3#^de887b]] $$\exists !y\in Y \quad\text{tal que} \quad\lVert x-y \rVert\leq \lVert x-u \rVert\quad \forall u\in Y$$
>>	2. Definimos $z=x-y$ entonces  $$\forall u\in Y\quad\lVert z-u \rVert=\lVert x-(y+u) \rVert\geq \lVert x-y \rVert=\lVert z \rVert$$ ($y+u\in Y$ por ser subespacio)
>>	3. Por [[Teórico 2#^ad0aac]] $z\in Y^{\perp}$
>>- Unicidad
>>	1. Suponemos $x=y_{1}+z_{1}=y_{2}+z_{2}$ entonces $y_{1}-y_{2}=z_{2}-z_{1}$ entonces $y_{1}-y_{2}\in Y$ y también $y_{1}-y_{2}\in Y^{\perp}$ entonces $y_{1}-y_{2}=0$ entonces $y_{1}=y_{2}$ entonces $x_{1}=x_{2}$ 
...
>>[!Proof]-
>>1. Primero, $Y\subseteq Y^{\perp\perp}$. Si $y\in Y$ y $v\in Y^{\perp}$ entonces $(y,v)=0$, por lo que $y\in Y^{\perp\perp}$.
>>2. Sea $x\in Y^{\perp\perp}$. Por la descomposición ortogonal ([[Teórico 3#^3686bc]]) existe única $y\in Y$ y $z\in Y^{\perp}$ tales que $x=y+z$.
>>3. Como $x\in Y^{\perp\perp}$, para todo $w\in Y^{\perp}$ se tiene $(x,w)=0$, en particular tomando $w=z$ obtenemos $(x,z)=0$.
>>4. Calculando, $(x,z)=(y+z,z)=(y,z)+(z,z)=0+\|z\|^{2}=\|z\|^{2}$. Por tanto $\|z\|^{2}=0$, y entonces $z=0$.
>>5. Entonces $x=y\in Y$, por lo que $Y^{\perp\perp}\subseteq Y$.
>>6. Juntando las inclusiones $Y\subseteq Y^{\perp\perp}$ y $Y^{\perp\perp}\subseteq Y$ concluimos $Y=Y^{\perp\perp}$.
>
...
>>[!Proof]-
>>1. $Y\subseteq \overline{Y}$ y como $Y\subseteq \overline{Y}$ entonces por [[Teórico 2#^7a502c]] inciso (e) tenemos    $\overline{Y}^{\perp}\subseteq Y^{\perp}$ 
>>2. Entonces $Y^{\perp\perp}\subseteq \overline{Y}^{\perp\perp}=\overline{Y}$ por [[Teórico 3#^31d10a]] 
>>3. Por otro lado por [[Teórico 2#^7a502c]] inciso (g)  $Y\subseteq Y^{\perp\perp}$  entonces $\overline{Y}\subseteq Y^{\perp\perp}$ (inclusion vale por que $Y^{\perp\perp}$ es  cerrado) 
...
>$X$ e.v.pi $\{ e_{n} \}\subset X$ sucesion ortonormal entonces $\forall x \in X$ la serie $\sum^{\infty}_{n=1} \lvert (x,e_{n}) \rvert^{2}$ converge y $$\sum^{\infty}_{n=1}\lvert (x,e_{n}) \rvert^{2}\leq\lVert x \rVert^{2}$$
>>[!Proof]-
>>1. Sea $y_{k} = \sum^{k}_{n=1}(x,e_{n})e_{n}$
>>2. Calculo de $\|x-y_k\|^2$ usando ortonormalidad. Entonces
>>$$\|x-y_k\|^2 = ( x-y_k, x-y_k ) = ( x,x) - ( x, y_k) - ( y_k, x) + ( y_k, y_k)$$
>>3. Ademas $( x, y_k) = \sum_{n=1}^k (x,e_n)\overline{(x,e_n)} = \sum_{n=1}^k |(x,e_n)|^2$ por lo tanto $( y_k, x) = \overline{( x, y_k)} = \sum_{n=1}^k |(x,e_n)|^2$
...
>[!Theorem] Series converge $\iff$ coeficientes estan en $\ell^{2}$  
...
>>2. $(x,e_{m})=\lim_{ k \to \infty }\left(\sum^{k}_{n=1}\alpha_{n}e_{n},e_{m}\right)=\alpha_{m}$ ($k\geq m$ cuando a partir de algun momento) Notar que aca usamos ortonormalidad
...
>>4. $(\Leftarrow)$ Sea $x_{k}=\sum^{k}_{n=1}\alpha_{n}e_{n}$ (Notar $x_{k}\in \mathcal{H}$  entonces por definicion tenemos una norma) 
>>5. $\lVert x_{k}-x_{j} \rVert=\left\lVert  \sum^{k}_{n=j+1}\alpha_{n}e_{n}  \right\rVert=(\sum^{k}_{n=j+1}\alpha_{n}e_{n},\sum^{k}_{n=j+1}\alpha_{n}e_{n})=\sum^{k}_{n=j+1}\lvert \alpha_{n} \rvert^{2} \xrightarrow[j,k \to \infty]{} 0$ (Por que la serie converge)
...
>Demo alternativa para la ida , esta bien? $$\sum^{\infty}_{n=1}\lvert \alpha_{n} \rvert^{2}=\sum^{\infty}_{n=1} \sum^{\infty}_{j=1}\alpha _{n}\overline{\alpha_{j}}(e_{n},e_{j}) =\left( \lim\limits_{ k \to \infty } \sum^{k}_{n=1} \alpha _{n}e_{n},\lim\limits_{ r \to \infty } \sum^{r}_{j=1} \alpha_{j}e_{j} \right)=\left( \sum^{\infty}_{n=1} \alpha _{n}e_{n},\sum^{\infty}_{j=1}\alpha _{j}e_{j}  \right)= \left\lVert  \sum^{\infty}_{n=1}\alpha_{n}e_{n}  \right\rVert^{2} \leq\infty$$
...
>>[!Proof]-
>>1. Tomamos $e_{1}\neq 0$ 
>>2. Supongamos $e_{1}=\sum^{\infty}_{n=1}\alpha_{2n}e_{2n}$ entonces $$\lVert e_{1} \rVert^{2}=(e_{1},e_{1})=\left( \sum^{\infty}_{n=1} \alpha_{2n}e_{2n},e_{1} \right)=0$$(Por que $\{ e_{n} \}$ es ortonormal con lo cual $(e_{2n},e_{1})=0\ \ \forall n\in \mathbb{N}$ ) lo que es absurdo  
...
>>[!Proof]-
>>$(1.\Rightarrow 4.)$ Sea $x \in\mathcal{H}$ e $y=x-\sum^{\infty}_{n=1}(x,e_{n})e_{n}$ entonces $\forall {m}\in\mathbb{N}$ vale $$(y,e_{m})=(x,e_{m})-\lim_{ N \to \infty } \left( \sum^{N}_{n=1} (x,e_{n})e_{n},e_{m} \right) = 0$$
...
>>$(2\Rightarrow 1)$ Sea $y\in\{ e_{n} \}^{\perp}$ entonces $(y,e_{n})=0\quad\forall {n}\in\mathbb{N}$ por lo tanto $e_{n}\in\{ y \}^{\perp} \ \ \forall n\in \mathbb{N}$ que sabemos es un subespacio cerrado por [[Teórico 2#^7a502c]]
>>$$Sp\{ e_{n} \}\subseteq\{ y \}^{\perp}\Rightarrow\mathcal{H}=\overline{Sp}\{ e_{n} \}\subseteq\overline{\{ y \}^{\perp}}=\{ y \}^{\perp}$$
>>Por lo tanto $y\in \{ y \}^{\perp}$ entonces $(y,y)=0$ mostrando que $y=0$ 
...
>- (a) Espacios normados de dim finita son separables
>- (b) Un Hilbert de dim infinita es separable $\iff$ tiene una BON de Hilbert
>>[!Proof]-
>>- (a) 
...
>>- (b)
>>	- $(\Rightarrow)$ 
>>		1. Tenemos $\{ x_{n} \}$ denso y numerable a partir de el armamos $\{ x_{n_{j}} \}$ dejando de lado los $x_{j}$ que son combinacion lineal de algun elemento anterior. 
>>		2. Por lo tanto $\{ x_{n_{j}} \}$ sera linealmente idependiente. 
>>		3. Aplicamos Grahm-Schmidt y obtenemos una sucesión ortonormal $\{ e_{n} \}$  $$Sp\{ e_{n} \}=Sp\{ x_{n_{j}} \}=Sp\{ x_{n} \}$$
>>		4. Como $\{ x_{n} \}$ era denso en $\mathcal{H}$ entonces  $$\overline{Sp}\{ e_{n} \}=\overline{Sp\{ e_{n} \}}=\overline{Sp\{ x_{n} \}}=\mathcal{H}$$ base ortonormal ([[Teórico 3#^f63eb1]])
>>	- $(\Leftarrow)$
>>		1. Ahora tenemos BON $\{ e_{n} \}$. Afirmamos que el conjunto $A$ de los $x\in\mathcal{H}$ que son expresables como sumas finitas $\sum^{k}_{n=1}\alpha_{n}e_{n}$ con $\alpha_{n}\in\mathbb{Q}$ (o $\mathbb{Q}+i\mathbb{Q})$ es denso y numerable.
>>		2. $A$ numerable es trivial.
>>		3. Sea $y\in\mathcal{H}$ entonces $y=\sum^{\infty}_{n=1}\beta_{n}e_{n}$ y sabemos que vale $\sum^{\infty}_{n=1}\lvert \beta_{n} \rvert^{2}\leq\infty$ ([[Teórico 3#^4e7cf9]]) entonces $\sum^{\infty}_{n=N+1}\lvert \beta_{n} \rvert^{2}< \frac{\epsilon^{2}}{2}$
>>		4. Para $n=1,\ldots,N$ elegimos $\alpha_{n}$ tales que $\lvert \beta_{n}-\alpha_{n} \rvert^{2}\leq \frac{\epsilon^{2}}{2N}\quad n=1,\ldots,N$ (Densidad en $\mathbb{Q}$) y definimos $\tilde{y}=\sum^{N}_{n=1}\alpha_{n}e_{n}$ entonces claramente $y\in A$  
>>		5. $\lVert y-\tilde{y} \rVert^{2}=\sum^{N}_{n=1}\lvert \beta_{n}-\alpha_{n} \rvert^{2}+\sum^{\infty}_{n=N+1}\lvert \beta_{n} \rvert^{2}\leq\epsilon^{2}$
>>		6. Entonces $A$ es denso en $\mathcal{H}$
...
>>2. Para el $L^{2}$ vamos a dar una BON en teorico 5. Base de Fourier  
^f743a7
...
>>[!Proof]-
>>3. Es facil ver que es Hilbert con $(f,g)=\sum^{}_{x\in\mathbb{R}}f(x)g(x)$ (Suma finita sale por Holder) 
>>4. Sea 
>>$$f_{y}(x) = \begin{cases} 1 & x=y \\ 0 & cc \end{cases}$$
>>Estan en el $\tilde{L}^{2}$. Y además hay no numerables de estas $f_{y}$ 
>>Ademas $\lVert f_{y}-f_{z} \rVert^{2}=2$. Si tuviese un denso numerable del $\tilde{L^{2}}$ para cada $f_{y}$ me armo una bola de radio mas chico que 2 cada una de estas bolas tiene algun elemento de dicho denso numerable, pero no se intersecan entre si. Absurdo porque tengo no numerables bolas.

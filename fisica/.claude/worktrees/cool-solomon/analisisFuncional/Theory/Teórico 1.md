...
>>[!Proof]-
>> 1. Sale usando $\lVert x \rVert\leq \lVert x-y \rVert+\lVert y \rVert$ y para la otra desigualdad $\lVert y \rVert\leq \lVert y-x \rVert+\lVert x \rVert=\lVert x-y \rVert+\lVert x \rVert$ 
>> 2. Sale usando 1
...
>>[!proof]-
>>1. Primero notar $M=(\sum_{j=1}^{n} \lVert e_{j} \rVert^2)^{\frac{1}{2}}>0$
>>2. Usando Hölder vemos que $$\lVert x \rVert=\left\lVert  \sum\alpha_{j}e_{j}  \right\rVert\leq \sum \lVert \alpha_{j}e_{j} \rVert=\sum\bigg||\alpha_{j}|\lVert e_{j} \rVert\bigg| \leq \big( \sum|\alpha_{j}|^2\big)^{\frac{1}{2}} \big(\sum\lVert e_{j} \rVert^2\big)^{\frac{1}{2}} = \lVert x \rVert_{c}M$$
>>(La desigualdad sale de usar $(\mathbb{N},\mu)$ donde $\mu$ es la medida de contar). Por lo tanto ya tenemos una desigualdad
...
>>4. Podemos ver que es continua $$\begin{align}\biggl\lvert \left\lVert  \sum\alpha_{j}e_{j}  \right\rVert-\left\lVert  \sum\beta_{j}e_{j} \right\rVert \biggl\rvert & \leq\|\sum\alpha_{j}e_{j}- \sum \beta_{j}e_{n}\|\\&=\|\sum(\alpha_{j}-\beta_{j})e_{j}\|\\&\leq \sum|\alpha_{j}-\beta_{j}|\|e_{j}\|\\&\leq ( \sum|\alpha_{j}-\beta_{j}|^2)^{\frac{1}{2}} (\sum\lVert e_{j} \rVert^2)^{\frac{1}{2}}\\&\leq \|\alpha_{j}-\beta_{j}\|_{2}M\end{align}$$ 
>>(Asumimos aca $(\mathbb{F}^{n},\|\cdot\|_{2})$) 
>>5. $S=\left\{  (\alpha_{1},\dots,\alpha_{n}):\sum|\alpha_{j}|^2 = 1  \right\}$ es compacto en $(\mathbb{F}^{n},\lVert \cdot \rVert_{2})$ 
...
>[!Lemma] $d_{c}$ es completo
> X espacio vectorial de dim finita y $\lVert \cdot \rVert_{c} =\left( \sum \lvert \alpha_{j} \rvert^2 \right)^{\frac{1}{2}}$ y $d_{c}$ la metrica asociada, entonces $(X,d_{c})$ es completo (Banach)
>>[!Proof]-
...
>> 4. Fijando $j\in \mathbb{N}$ tenemos $\lvert \alpha_{j}^k-\alpha_{j}^m \rvert\leq\epsilon\quad \forall k,m\geq m_{0}$
>> 5. $\{ \alpha_{j}^n \}_{n\in \mathbb{N}}$ es de cauchy en un completo $\mathbb{F}$ entonces tiene limite $\alpha_{j}$ (vale para cada $j$)
>> 6. $\exists n_{j}\in \mathbb{N}$ tq $\lvert \alpha_{j}^n -\alpha_{j} \rvert < \frac{\epsilon}{N} \quad\forall n\geq n_{j}$ 
...
>> 9. para $n\geq \tilde{n}$ (usando 6.) tenemos $$\lVert x^{n}-x \rVert_{c}^2=\sum_{j}^N\lvert \alpha _{j}^n-\alpha_{j} \rvert^2\leq \epsilon$$ Mostrando que $\{ x^{n} \}$ converge a $x$ 
...
>>[!Proof]-
>>4. $S=\bigg\{ \{ x_{n} \} \subseteq\ell^{\infty}:\exists n_{0}\in \mathbb{N} \ / \ x_{n}=0 \ \forall n\geq n_{0} \bigg\}$
...
>>[!Proof]-
>>1. Tomamos $x\in X\setminus Y$ cualquiera. Luego  $d=\inf\{ \lVert x-z \rVert:z\in Y \}>0$ (mayor estricto por que $Y$ cerrado) 
...
>>[!Proof]-
>>1. $\sum^m_{k=n+1} \lVert x_{k} \rVert=\bigg\lvert  \sum^m \lVert x_{k} \rVert - \sum^n \lVert x_{k} \rVert \bigg\rvert  \leq\epsilon \quad\forall m\geq n\geq n_{0}$ (por ser convergente la serie, las sumas parciales son de cauchy)
